...
@section('js')
    <script>
        ...
        function setMonitoring() {
            monitoring = perangkats.map((perangkat) => {
                ...
                <div style="display: flex; align-items: center;">
                    <img src="${userImageUrl}" alt="Logo" style="width: 25px; height: 25px; margin-right: 20px;">
                        <div>
                            ...
                            <div class="labelstatus" id="status-${perangkat.id}">${statusContent}</div>
                        </div>
                </div>
            `;

                perangkat.popupContent = popupContent;
                ...
                var marker = L.marker([perangkat.latitude, perangkat.longitude], {
                    draggable: true,
                    // icon: markerIcon
                }).bindPopup(perangkat.popupContent, {
                    autoClose: false,
                    closeOnClick: false,
                });
            ...
            monitoring = perangkats.map(perangkat => {
                $.ajax({
                    ...
                    success: function(status) {
                        if (status == true) {
                            perangkat.statusContent = 'terhubung';
                            $("[id='status-" + perangkat.id + "']").html(
                                "<span class='badge bg-success'>Terhubung</span>")
                        } else {
                            perangkat.statusContent = 'tidak terhubung';
                            $("[id='status-" + perangkat.id + "']").html(
