...
@section('js')
    ...
    <script>
        ...
        var monitoring = servers.map((server) => {
            // Generate the route URL directly in the JavaScript
            var detailKlienUrl = `{{ route('detailKlien', ['id' => '__ID__']) }}`.replace('__ID__', server.id);
            // Using HTML span with Bootstrap badge class to display device_count and adding a link
            var popupContent = `
                <div style="display: flex; justify-content: center; align-items: center; height: 100%;">
                    <div style="text-align: center;">
                        <strong>${server.nama_server}</strong><br>
                        <a href="${detailKlienUrl}" class="badge bg-primary">Klien: ${server.users_count}</a>
                    </div>
                </div>
            `;

            var marker = L.marker([server.latitude, server.longitude], {
                draggable: true
